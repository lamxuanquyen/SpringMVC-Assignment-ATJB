...
<form id="pageSizeForm" action="${contextPath}/sudungmay/list" method="get">
	...
	<ul class="pagination d-flex justify-content-end mr-4">
		<c:if test="${pageNumber > 1}">
			<li class="page-item"><a class="page-link"
				href="${contextPath}/sudungmay/list?pageSize=${pageSize}&pageNumber=${pageNumber-1}&search=${search}">Previous</a></li>
		...
		<c:forEach begin="1" end="${totalPages}" var="i">
			<c:choose>
				...
				<c:otherwise>
					<li class="page-item"><a class="page-link"
						href="${contextPath}/sudungmay/list?pageSize=${pageSize}&pageNumber=${i}&search=${search}">${i}</a></li>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		...
		<c:if test="${pageNumber lt totalPages}">
			<li class="page-item"><a class="page-link"
				href="${contextPath}/sudungmay/list?pageSize=${pageSize}&pageNumber=${pageNumber+1}&search=${search}">Next</a></li>
		</c:if>
	</ul>
